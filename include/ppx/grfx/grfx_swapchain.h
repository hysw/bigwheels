...
public:
    ...
    Result SetIndirectRenderSize(uint32_t width, uint32_t height);
...
protected:
    ...
    struct Target
    {
        ...
        Result WrapColorImages(grfx::Device&, grfx::Format format, const std::vector<void*>& handles);
        Result CreateColorImages(grfx::Device&, grfx::Format format, uint32_t imageCount);
        void   DestroyColorImages(grfx::Device&);

        Result WrapDepthImages(grfx::Device&,  grfx::Format format, const std::vector<void*>& handles);
        Result CreateDepthImages(grfx::Device&,  grfx::Format format, uint32_t imageCount);
        void   DestroyDepthImages(grfx::Device&);

        Result CreateRenderPasses(grfx::Device&);
        void   DestroyRenderPasses(grfx::Device&);
...
private:
    ...
    Result CreateTargetImpl(Target&, uint32_t width, uint32_t height, const std::vector<void*>* pColorHandles, const std::vector<void*>* pDepthHandles);
    Result CreateTarget(Target&, uint32_t width, uint32_t height, const std::vector<void*>* pColorHandles, const std::vector<void*>* pDepthHandles);
    void DestroyTarget(Target&);

protected:
    grfx::QueuePtr mQueue;
    Target         mIndirectTarget;
    Target         mDeviceTarget;
    bool           mIsIndirect = false;
    const Target&  GetTarget() const { return mIsIndirect ? mIndirectTarget : mDeviceTarget; }
    Target&        GetTarget() { return mIsIndirect ? mIndirectTarget : mDeviceTarget; }

    Result CreateIndirectTarget(uint32_t width, uint32_t height);
    void   DestroyIndirectTarget();
    Result CreateDeviceTarget(const std::vector<void*>* pColorHandles, const std::vector<void*>* pDepthHandles);
    void   DestroyDeviceTarget();
